...
- (void)didConnect
{
    ...
    assert(SecRandomCopyBytes(kSecRandomDefault, keyBytes.length, keyBytes.mutableBytes) == 0);
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data
{
    ...
    if (!useMask) {
        ...
        assert(SecRandomCopyBytes(kSecRandomDefault, sizeof(uint32_t), (uint8_t *)mask_key) == 0);
